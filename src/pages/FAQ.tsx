...
const FAQ = () => {
  ...
  const handleCitySubmit = async (e: React.FormEvent) => {
    ...
    try {
      const {
        error
      } = await supabase.from('city_suggestions').insert({
  ...
  const handleOpportunitySubmit = async (e: React.FormEvent) => {
    ...
    try {
      const {
        error
      } = await supabase.from('opportunity_type_suggestions').insert({
        opportunity_type: opportunityInput.trim()
      });
      if (error) throw error;
      toast({
  ...
  const renderFAQContent = (faq: any, index: number) => {
    ...
    return <AccordionContent className="px-4 pb-4 pt-2">
        ...
        {isCityQuestion && <form onSubmit={handleCitySubmit} className="space-y-3">
            <Input type="text" placeholder="Enter your city name..." value={cityInput} onChange={e => setCityInput(e.target.value)} className="w-full" disabled={citySubmitting} />
            <Button type="submit" className="intro-button-cta" disabled={citySubmitting || !cityInput.trim()}>
              {citySubmitting ? 'Submitting...' : 'Submit City'}
            </Button>
          </form>}
        
        {isOpportunityQuestion && <form onSubmit={handleOpportunitySubmit} className="space-y-3">
            <Input type="text" placeholder="Enter opportunity type (e.g., Job referrals, Product reviews...)..." value={opportunityInput} onChange={e => setOpportunityInput(e.target.value)} className="w-full" disabled={opportunitySubmitting} />
            <Button type="submit" className="intro-button-cta" disabled={opportunitySubmitting || !opportunityInput.trim()}>
  ...
  return <div className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pt-24">
        ...
        <div className="flex flex-wrap gap-2 justify-center mb-8">
          {categories.map(category => <Badge key={category.id} variant={activeCategory === category.id ? "default" : "outline"} className={`cursor-pointer transition-all hover:shadow-md px-4 py-2 ${activeCategory === category.id ? 'bg-primary text-primary-foreground shadow-lg' : 'hover:bg-accent'}`} onClick={() => setActiveCategory(category.id)}>
              {category.label}
            </Badge>)}
        </div>
          ...
          <div className="lg:w-1/4">
            <div className="intro-card p-6 sticky top-8 hidden lg:block">
              ...
              <div className="space-y-2">
                {categories.map(category => <button key={category.id} onClick={() => setActiveCategory(category.id)} className={`w-full flex items-center justify-between p-3 rounded-lg transition-all text-left ${activeCategory === category.id ? 'bg-primary text-primary-foreground shadow-md' : 'hover:bg-accent text-foreground'}`}>
                    <span className="font-medium">{category.label}</span>
                    <Badge variant="secondary" className={`text-xs ${activeCategory === category.id ? 'bg-primary-foreground text-primary' : 'bg-destructive text-destructive-foreground'}`}>
                      {category.count}
                    </Badge>
                  </button>)}
              </div>
          ...
          <div className="lg:w-3/4">
            <div className="intro-card p-6">
              ...
              <Accordion type="single" collapsible className="w-full">
                {faqs[activeCategory as keyof typeof faqs]?.map((faq, index) => <AccordionItem key={index} value={`item-${index}`} className="border-border">
        ...
        <div className="mt-16 w-full animate-fade-in overflow-hidden">
          <img src="/lovable-uploads/c87dab29-5f64-49aa-bd17-0040b75b0c38.png" alt="Where matchmaking earns you money" className="w-full h-auto object-cover scale-95 -mb-8" />
        </div>
